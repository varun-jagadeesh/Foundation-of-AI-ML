...
class ExactInference(InferenceModule):
        ...
        for p in self.legalPositions:
            if noisyDistance == None:
                ...
                if emissionModel[trueDistance] > 0:
                    pos[p] = self.beliefs[p] * emissionModel[trueDistance]

        if noisyDistance == None:
            pos[self.getJailPosition()] = 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            newDistance = self.getPositionDistribution(self.setGhostPosition(gameState,p))
            for k,v in newDistance.iteritems():
                pos[k] = pos[k] + v * self.beliefs[p]
        pos.normalize()
        self.beliefs = pos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for count in range(0,self.numParticles):
            self.listOfParticles.append(self.legalPositions[count % len(self.legalPositions)])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for count in range(0,self.numParticles):
                particleUpdate.append(self.getJailPosition())
            self.listOfParticles = particleUpdate
        else:
            particleWeight = util.Counter()
            for particle in self.listOfParticles:
                distance = util.manhattanDistance(particle,pacmanPosition)
                particleWeight[particle] = particleWeight[particle] + emissionModel[distance]
            if particleWeight.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                particleWeight.normalize()
                for count in range(0,self.numParticles):
                    particleUpdate.append(util.sample(particleWeight))
                self.listOfParticles = particleUpdate
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        particleUpdate = []
        for oldPos in self.listOfParticles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            particleUpdate.append(util.sample(newPosDist))
        self.listOfParticles = particleUpdate
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for count in range(0, self.numParticles):
            self.listOfParticles.append(permutation[count % len(permutation)])
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                self.listOfParticles = [self.getParticleWithGhostInJail(p, i) for p in self.listOfParticles]
        ...
        for particle in self.listOfParticles:
            ...
            for i in range(self.numGhosts):
                if particle[i] != self.getJailPosition(i):
                    ...
                    evidence = evidence * emissionModels[i][distance]
            particleWeight[particle] += evidence
        particleWeight.normalize()
        if particleWeight.totalCount() == 0:
            self.initializeParticles()
            for i in range(self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.listOfParticles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                ghostPos = setGhostPositions(gameState, list(oldParticle))
                newPosDist = getPositionDistributionForGhost(ghostPos, i, self.ghostAgents[i])
                newParticle[i] = util.sample(newPosDist)
            "*** END YOUR CODE HERE ***"
            particleUpdate.append(tuple(newParticle))
        self.listOfParticles = particleUpdate
